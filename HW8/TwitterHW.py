...
import re
import tweepy
import twitter_info2 # still need this in the same directory, filled out

consumer_key = twitter_info2.consumer_key
consumer_secret = twitter_info2.consumer_secret
access_token = twitter_info2.access_token
access_token_secret = twitter_info2.access_token_secret
...
cur.execute('CREATE TABLE Tweets(tweet_id TEXT, author TEXT, time_posted TIMESTAMP,tweet_text TEXT, retweet NUMBER)')
...
for tw in umsi_tweets:
    tup= tw['id'], tw['user']['screen_name'], tw['created_at'], tw['text'], tw['retweet_count'] #used in project
    cur.execute('INSERT INTO Tweets (tweet_id, author, time_posted, tweet_text, retweet) VALUES (?,?,?,?,?)',tup)
...
# Print the results
cur.execute('SELECT author FROM Tweets WHERE retweet > 2') #set(list) gives
