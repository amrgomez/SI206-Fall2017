...
## Any names of people you worked with on this assignment:Kayla Williams
...
##		to search for is.
def twitter_data(req):
    if req in CACHE_DICTION:
        print("cached")
        return CACHE_DICTION[req]
    else:
        print('Making new request')
        response = api.search(req)
        try:
            CACHE_DICTION[req] = response
...
for i in range(3):
    ...
    tweet_result= twitter_data(search)
